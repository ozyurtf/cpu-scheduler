...
After a new process is called with fork(), the exec() system call is made to execute the program in the new process. 

And if exceptions happen (e.g., division by zero, illegal memory access, segmentation faults, etc.), user enters an input, process is terminated, timer set by the process expires, etc., signals are sent to notify processes about these events.  

Signals are interrupts that are generated by the CPU after executing specific instructions/conditions within the software or program code. CPU generates these interrupts as a result of encountering specific conditions during the execution of the software, and a signal is sent to the process to notify the process about these specific conditions/events.
