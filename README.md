...
- registers *(hardware resource that can hold any kind of data that is necessary for the execution)*
...
- When the system is initialized.
      - When a computer initializes hardware devices and the operationg system and when it loads the operating system into the memory, new processes are be created.
...
In multiprogramming, there are multiple processes that are residing in the memory and sharing the resources such as CPU and memory space. And therefore CPU can be able to switch from one process to another process quickly. It lets processes to use the CPU when it would otherwise become idle because having multiple processes in the memory and switching between them is a great way to prevent the CPU being idle, keep it busy, and as a result obtain higher throughput.

Also, sharing resources increases resource utilization. Through multiprogramming, we can also ensure about the fairness among the processes.

Because multiple processes reside in the memory, a mechanism that will pick one of these processes and give it to the CPU to be executed is needed. And through this mechanism, one proecss is picked and executed. And in multiprogramming, the order these processes are picked and the amount of CPU time that is alloced to these processes will vary across different runs. Therefore, if we run the same program several times, we don't observe the same execution times because of **multiprogramming** and also **interrupts**. 

If you don't know what the interrupt is: Interrupt is basically a signal/notification that is sent by hardware or software to the CPU to suspend the execution of a process temporarily. It indicates that an event happened that requires attention. 
...
So, we have mentioned that multiprogramming increses CPU utilization how we calculate the CPU utilization ? 
