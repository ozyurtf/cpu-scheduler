...
Signals are interrupts that are generated by the CPU after executing specific instructions/conditions within the software or program code. They are used for **inter-process communication**. They are sent from one process to another, or from kernel to process to notify the process about an event. 
...
CPU generates these interrupts as a result of encountering specific conditions during the execution of the software, and a signal is sent to the process to notify the process about these specific conditions/events.

For example, during the execution of a process, if exceptions happen (e.g., division by zero, illegal memory access, segmentation faults, etc.) user enters an input, process is terminated, or timer set by the process expires, etc., signals are sent to notify processes about these events. 

**!Slide 27 is skipped!**

**Summary**: fork(), exec(), wait() and its variants are the operating system's system calls that are used to create and manipulate processes. Process IDs are **handles** that are used to identify the processes. Handle in here means reference to the resource and it is abstracted which means that it represents the resource in a way that it is not tied to the physical resource directly. And the the resources can be represented in an abstract way by using an integer, pointer, pointer to pointer, pointer to integer, string, etc. 

Here is an example of a handle in C 

```
#include <stdio.h>

int main() {
    FILE* file = fopen("example.txt", "r"); // File handle
    // ...
