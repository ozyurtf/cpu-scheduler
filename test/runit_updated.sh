...
usage() {
    echo "usage: $0 <outputdir> <scheduler+optionalargs> <rfile>"
    exit 1
}
...
[[ ! -x ${PROG} ]]   && echo "program <$PROG> is not executable" && exit 1
[[ ! -d ${OUTDIR} ]] && echo "outputdir <$OUTDIR> does not exist"  && exit 1
[[ ! -d ${INDIR} ]]  && echo "inputdir <$INDIR> does not exist"  && exit 1
...
for f in ${INS}; do
    for s in ${SCHEDS}; do 
        ...
        monitor "${OUTF}" "${SCHED}" -s"${s}" "input${f}" "${@: -1}"
    done
done
